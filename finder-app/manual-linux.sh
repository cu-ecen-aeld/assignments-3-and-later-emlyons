...
if [ ! -e ${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image ]; then
    ...
    make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} defconfig # default config? story says to config virt devboard
    make -j4 ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} Image # build kernel image
fi
